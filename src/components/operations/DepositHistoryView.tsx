...
export const DepositHistoryView: React.FC = () => {
  const { depositHistory, setInspectingTransaction, agents } = useAppStore();
  ...
  const filteredDeposits = useMemo(() => {
    return depositHistory.filter((tx) => {
      const matchSearch =
        ...
        (tx.targetWalletId && tx.targetWalletId.toLowerCase().includes(searchTerm.toLowerCase())) ||
  ...
  }, [depositHistory, searchTerm, statusFilter, selectedAgentFilter]);

  const totalVolume = filteredDeposits.reduce((acc, t) => acc + (t.status === 'Approved' ? t.amount : 0), 0);
  const totalCommission = filteredDeposits.reduce((acc, t) => acc + (t.commissionEarned || 0), 0);

  return (
    <div id="deposit-history-view" className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
      {/* Header Bar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200 pb-4">
        <div>
          <h1 className="text-xl font-bold text-slate-900">سجل عمليات الإيداع (Deposit History)</h1>
          <p className="text-xs text-slate-500 mt-1">
            الأرشيف المالي الشامل لجميع طلبات الإيداع المكتملة والمرفوضة
          </p>
        </div>
        <div className="flex items-center gap-4 text-xs">
          <div className="bg-slate-100 px-3 py-1.5 rounded border border-slate-300">
            <span className="text-slate-500">إجمالي الحجم المعتمد:</span>{' '}
      ...
      <div className="flex flex-wrap items-center gap-3 bg-white p-3 rounded-lg border border-slate-200 text-xs">
        <input
          ...
          type="text"
          placeholder="بحث بالمعرف، اسم المستخدم، المحفظة، الوكيل..."
          value={searchTerm}
        ...
        <span className="text-slate-500 font-medium ml-auto">
          العدد: {filteredDeposits.length} عملية
        </span>
      </div>
              ...
              {filteredDeposits.length === 0 ? (
                <tr>
                  <td colSpan={10} className="p-8 text-center text-slate-400">
                    لا توجد عمليات إيداع مطابقة للفلاتر المحددة.
                  </td>
                </tr>
              ) : (
                filteredDeposits.map((tx) => (
                  <tr key={tx.id} className="hover:bg-slate-50 transition">
                    ...
                    <td className="p-3 font-mono text-slate-600">{tx.targetWalletId || tx.phone || 'N/A'}</td>
                    <td className="p-3">
